...
export default function PersonalDevices(){
  ...
  const [name, setName] = useState(""); const [platform, setPlatform] = useState("android");
  const [err, setErr] = useState(""); const [busy, setBusy] = useState(false);
  const [editId, setEditId] = useState(null); const [editName, setEditName] = useState("");
  const load = async ()=>{
    setErr("");
    const { data:d1, error:e1 } = await supabase.from("devices").select("id,name,platform,is_active").order("created_at",{ascending:false});
  ...
  async function addDevice(){
    ...
    if (error) setErr(error.message); else { setName(""); setPlatform("android"); await load(); }
    setBusy(false);
  }
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl bg-gray-900 ring-1 ring-gray-800 p-5">
        <div className="grid sm:grid-cols-3 gap-3">
          ...
          <button disabled={busy || !name} onClick={addDevice} className="rounded px-4 py-2 bg-lime-400 text-black font-semibold disabled:opacity-50">Add</button>
        </div>
